    ...
    return (<div className="space-y-3">
      ...
      <div className="flex justify-center gap-2">
        <div className="flex gap-0.5 p-0.5 bg-muted rounded-lg w-fit">
          <button type="button" onClick={() => onFetchTypeChange?.("single")} className={cn("flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium transition-all cursor-pointer", fetchType === "single"
            ? "bg-background text-foreground shadow-sm"
            : "text-muted-foreground hover:text-foreground")}>
            <User className="h-3 w-3"/>
            Single
          </button>
          <button type="button" onClick={() => onFetchTypeChange?.("multiple")} className={cn("flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium transition-all cursor-pointer", fetchType === "multiple"
            ? "bg-background text-foreground shadow-sm"
            : "text-muted-foreground hover:text-foreground")}>
      ...
      {fetchType === "multiple" && (<div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <Button id="import-txt" variant="outline" onClick={onImportFile} className="flex items-center gap-2 flex-1">
            ...
            {isFetchingAll && (<Button variant="destructive" onClick={onStopAll} className="flex items-center gap-2">
      ...
      {fetchType === "single" && (mode === "public" || isLikesMode) && (<div className="space-y-2">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              {username && (<button type="button" className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors cursor-pointer" onClick={() => onUsernameChange("")}>
      ...
      {showAuthInput && (<div className="p-3 border rounded-lg bg-muted/30">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Tooltip>
                ...
                <TooltipContent className="text-center">
                  {mode === "private" ? (<p>Use auth token from the account whose bookmarks/likes you want to fetch</p>) : (<p>Recommended to use a dummy account, not your main account.<br />Excessive usage may cause suspension</p>)}
                </TooltipContent>
              </Tooltip>
            </div>
            <div className="relative flex-1">
              <InputWithContext id="auth-token" type={mode === "public"
                ? showPublicToken
                    ? "text"
                    : "password"
                : showPrivateToken
                    ? "text"
                    : "password"} placeholder="Enter your auth_token cookie value" value={currentAuthToken} onChange={(e) => mode === "public"
      ...
      {showAdvanced && (<div className="p-3 border rounded-lg bg-muted/30 space-y-3">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {mode === "public" && (<>
                <div className="flex items-center gap-2">
                  <Checkbox id="date-range" checked={useDateRange} onCheckedChange={(checked) => setUseDateRange(checked as boolean)} className="bg-background"/>
      ...
      {fetchType === "multiple" && multipleAccounts.length > 0 && (<div className="p-3 border rounded-lg bg-muted/30">
            ...
            {multipleAccounts.map((account) => (<div key={account.id} className="p-3 border rounded-lg bg-card">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    {account.accountInfo ? (<>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {account.showDiff && account.previousMediaCount > 0 && account.mediaCount > account.previousMediaCount && (<span className="text-sm text-green-600 dark:text-green-400 font-medium">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {account.status === "pending" && (<span className="text-xs px-2 py-1 bg-gray-500/20 text-gray-600 dark:text-gray-400 rounded flex items-center gap-1.5">
                        <Hourglass className="h-3 w-3"/>
                        Pending
                      </span>)}
                    {account.status === "completed" && (<span className="text-xs px-2 py-1 bg-green-500/20 text-green-600 dark:text-green-400 rounded flex items-center gap-1.5">
