...
export function SettingsPage() {
    ...
    const [extractorInstalled, setExtractorInstalled] = useState(false);
    ...
    useEffect(() => {
        const loadDependencyStatus = async () => {
            try {
                const [extractor, ffmpeg, exiftool] = await Promise.all([
                    IsExtractorInstalled(),
                    IsFFmpegInstalled(),
                    IsExifToolInstalled(),
                ]);
                setExtractorInstalled(extractor);
    ...
    const handleDownloadExtractor = async () => {
        ...
        try {
            ...
            setExtractorInstalled(true);
            toast.success(extractorInstalled ? "Xtractor reinstalled successfully" : "Xtractor downloaded successfully");
        }
        catch (error) {
        ...
        <div className="space-y-4">
            ...
            <div className="flex min-h-9 items-center gap-3">
              ...
              <Button variant="outline" size="sm" className="h-9" onClick={handleDownloadExtractor} disabled={downloadingExtractor}>
                {downloadingExtractor ? (<>
                    ...
                    {extractorInstalled ? "Reinstall Xtractor" : "Download Xtractor"}
                  </>)}
              </Button>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="h-9 flex items-center">
              {exiftoolInstalled ? (<div className="flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
                  <Check className="h-4 w-4"/>
                  Installed
                </div>) : (<Button variant="outline" size="sm" className="h-9" onClick={handleDownloadExifTool} disabled={downloadingExifTool}>
                  {downloadingExifTool ? (<>
                      <Spinner />
                      Downloading...
                    </>) : (<>
          ...
          <div className="space-y-2">
            ...
            <div className="h-9 flex items-center">
              {ffmpegInstalled ? (<div className="flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
                  <Check className="h-4 w-4"/>
                  Installed
                </div>) : (<Button variant="outline" size="sm" className="h-9" onClick={handleDownloadFFmpeg} disabled={downloadingFFmpeg}>
                  {downloadingFFmpeg ? (<>
                      <Spinner />
                      Downloading...
                    </>) : (<>
          ...
          {ffmpegInstalled && (<div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={tempSettings.gifResolution} onValueChange={(value: GifResolution) => setTempSettings((prev) => ({ ...prev, gifResolution: value }))}>
          ...
          <div className="space-y-2">
            ...
            <InputWithContext id="proxy" value={tempSettings.proxy || ""} onChange={(e) => setTempSettings((prev) => ({ ...prev, proxy: e.target.value }))} placeholder="http://proxy:port or socks5://proxy:port (optional)" className="w-[90%]"/>
          </div>
